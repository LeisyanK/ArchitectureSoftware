...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        // к портам подключаем внешние адаптеры
        // к порту IFetchMovieReviews подключаем адаптер MovieReviewsRepo
        IFetchMovieReviews fetchMovieReviews = new MovieReviewsRepo();
        // к порту IPrintMovieReviews подключаем адаптер ConsolePrinter
        IPrintMovieReviews printMovieReviews = new ConsolePrinter();

        // к порту IUserInput подключаем адаптер UserCommand, который принимает 
        // в себя два порта, т.е. адаптер внешний знает о существовании двух внутренних портов,
        // и он подключается к порту
        // (передаем ссылки на внешние адаптеры через внутренние порты)
        IUserInput userCommand = new UserCommand(fetchMovieReviews, printMovieReviews);

        MovieUser movieUser = new MovieUser(userCommand);
        
        // отправляем запрос на поиск треков
